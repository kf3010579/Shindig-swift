...
GADInterstitialDelegate
{
    ...
    @IBOutlet weak var awesomePeeps: UILabel!
...
override func viewDidLoad() {
    ...
    var dy : CGFloat = self.topView.frame.height * ratio - MENUMARGIN * 2
...
//    println("dx = \(dx)")
//    println("dy = \(dy)")
//    println("x = \(self.view.frame.width - dx - MENUMARGIN)")
//    println("y = \(MENUMARGIN)")
//    println("ratio = \(ratio)")
...
func setupCategoriesScrollView() {
        ...
        for itemCount = 0; itemCount < categoriesArray.count; itemCount++ {
            ...
            categoriesScrollView.addSubview(getCellView(xCoord, y: yCoord, wid: width, hei: height, imageicon:img, imgName: "\(categoriesArray[itemCount])", f: true, f1: false, index:NSInteger(itemCount)))
...
//downloading addata(posting) from Parse DB in ScrollView(Category, Spotlight, Closeby)
    func getCellView(x:CGFloat, y:CGFloat, wid:CGFloat, hei:CGFloat, imageicon:UIImage, imgName:String, f:Bool, f1:Bool, index:NSInteger) -> UIView{
        ...
        if f == true{
            ...
            myButton.addTarget(self, action: "adButtonClicked:", forControlEvents: UIControlEvents.TouchUpInside)
            if f1 == true{
                myButton.tag = 2 * index
            }
            else{
                myButton.tag = 2 * index + 1
            }
        }
            ...
            if error == nil {
                if let objects = objects as? [PFObject] {
                    ...
                    for object in objects {
                        ...
                        self.setupSpotlightScrollView(object, index: i)
                        ++i
                    }
                }
                println("Spot By Array count : \(self.spotLightArray.count)")
//                if self.spotLightArray.count > 0 {
//                    self.setupSpotlightScrollView(<#object: PFObject#>, index: <#NSInteger#>)
//                }
            ...
            if error == nil {
                if let objects = objects as? [PFObject] {
                    ...
                    for object in objects {
                        ...
                        self.setupCloseByScrollView(object, index: i)
    ...
    func setupSpotlightScrollView(object: PFObject, index: NSInteger) {
...
//    for itemCount = 0; itemCount < self.spotLightArray.count; itemCount++ {
        var item: PFObject = object//spotLightArray[index] as! PFObject
        
        let bgImg = item[CLASSIF_IMAGE1] as! PFFile
        bgImg.getDataInBackgroundWithBlock({ (imageData:NSData?, error:NSError?) -> Void in
            if(error == nil){
                let image = UIImage(data:imageData!)
                xCoord =  (width + gapBetweenButtons) * CGFloat(index) + gapBetweenButtons
                self.spotlightScrollView.addSubview(self.getCellView(xCoord, y: yCoord, wid: width, hei: height, imageicon:image!, imgName: "$\(item[CLASSIF_PRICE] as! String)", f: false, f1: true, index:NSInteger(index)))
            }
        })
...
//    } // END LOOP ================================
    ...
    // Place Buttons into the ScrollView =====
    spotlightScrollView.contentSize = CGSizeMake( (width + 15) * CGFloat(index + 1), yCoord)

}
    
func setupCloseByScrollView(object: PFObject, index: NSInteger) {  //Close By Scroll View
    var xCoord: CGFloat = 5
    var yCoord: CGFloat = 5
...
//    for itemCount = 0; itemCount < self.closeByArray.count; itemCount++ {
        var item: PFObject = closeByArray[index] as! PFObject
        ...
        bgImg.getDataInBackgroundWithBlock({ (imageData:NSData?, error:NSError?) -> Void in
            if(error == nil){
                let image = UIImage(data:imageData!)
                xCoord =  (width + gapBetweenButtons) * CGFloat(index) + gapBetweenButtons
                self.closebyScrollView.addSubview(self.getCellView(xCoord, y: yCoord, wid: width, hei: height, imageicon:image!, imgName: "$\(item[CLASSIF_PRICE] as! String)", f: false, f1: false, index:NSInteger(index)))
            }
        })
        // Add Buttons & Labels based on xCood
...
//    }  //END LOOP ================================
    ...
    // Place Buttons into the ScrollView =====
    closebyScrollView.contentSize = CGSizeMake( (width + 15) * CGFloat(index + 1), yCoord)
...
func catButtTapped(sender: UIButton) {
    ...
    query.findObjectsInBackgroundWithBlock { (objects, error)-> Void in
        if error == nil {
            ...
            // Go to Browse Ads VC
            let baVC = self.storyboard?.instantiateViewControllerWithIdentifier("BrowseAdID") as! BrowseAd
...
func adButtonClicked(sender: UIButton) {
    
    var index = sender.tag / 2
    var ext = sender.tag % 2
    ...
    if ext == 0{//Spotlight Button
        classifClass = spotLightArray[index] as! PFObject
    }else{//CloseBy
        classifClass = closeByArray[index] as! PFObject
    }
    
    if PFUser.currentUser() != nil{//Favorite Ad Or not
        var obj = PFObject(className: FAV_CLASS_NAME)
        for obj in favAdsArray{
            var mid = obj[FAV_AD_POINTER] as! PFObject
            println("obj..............\(mid.objectId)")
            println("classifClass.....\(classifClass.objectId)")
            if classifClass.objectId == mid.objectId{
                singleAdVC.favFlag = true
                break
            }
        }
    }
    ...
    //You can get the number of the members from Parse DB
    func getAwesomePeeps(){
      self.awesomePeeps.text = "..."
        var query = PFQuery(className:USER_CLASS_NAME)
        query.whereKey(USER_USERNAME, equalTo: "test")
    ...
    //You can get the favorite ad data from Parse DB
...
@IBAction func searchButt(sender: AnyObject) {
    ...
    query.findObjectsInBackgroundWithBlock { (objects, error)-> Void in
        if error == nil {
            ...
            // Go to Browse Ads VC
             let baVC = self.storyboard?.instantiateViewControllerWithIdentifier("BrowseAds") as! BrowseAd
                self.navigationController?.pushViewController(baVC, animated: true)
                //self.presentViewController(baVC, animated: true, completion: nil)
